...
begin
  ...
  select
    ...
    item.unit_price,
    case
      when coalesce(item.is_discountable, item.line_type = 'base', true)
      then item.discount_percent
      else 0
    end,
    coalesce(item.line_type, 'base'),
    coalesce(item.is_discountable, coalesce(item.line_type, 'base') = 'base'),
    item.notes
  ...
  set workflow_status = 'sales_quotation_created'
  where p.id = p_project_id
    and p.workflow_status = 'sales_client_created';
